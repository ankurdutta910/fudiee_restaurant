...
function AddItem() {
  ...
  //     const storageRef = ref(storage, `items/${id}/${file.name}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (file) {
        ...
        const name = new Date().getTime() + file.name;
        const storageRef = ref(storage, `items/${id}/${name}`);
      ...
      if (id) {
        // Add data to the "items" subcollection under the restaurant document
        await addDoc(collection(db, "restaurants", id, "items"), {
          ...data,
          img: imageUrl, // Add the uploaded image URL to the document
        ...
        toast.error(
          "No restaurant ID found. Please ensure user data is loaded."
        );
      }
    } catch (error) {
      console.error("Error adding item to subcollection:", error);
